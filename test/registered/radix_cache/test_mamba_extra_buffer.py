...
class TestMambaExtraBufferUnit(unittest.TestCase):
    ...
    def test_extra_buffer_alloc(self):
        req = _make_req()
        self.pool.alloc([req])
        self.assertIsNotNone(getattr(req, "mamba_ping_pong_track_buffer", None),
...
class TestMambaExtraBufferServer(unittest.TestCase):
    ...
    def test_server_inference_extra_buffer_mode(self):
        import requests
        payload = {"model": "default", "messages": [{"role": "user", "content": "What is 2+2?"}],
        ...
        baseline = requests.post(f"{baseline_url}/v1/chat/completions", json=payload, timeout=60)
