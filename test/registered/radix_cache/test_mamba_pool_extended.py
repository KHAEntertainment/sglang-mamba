...
def _make_pool(max_num_reqs=10, mamba_cache_size=20, max_context_len=128,
    ...
    return HybridReqToTokenPool(
        ...
        enable_mamba_extra_buffer=enable_extra_buffer, speculative_num_draft_tokens=3,
    )


def _make_req():
    return Req(rid=0, origin_input_text="", origin_input_ids=[],
...
class TestMambaPoolExtended(unittest.TestCase):
    ...
    def tearDown(self):
        self.assertEqual(self.pool.mamba_pool.available_size(), self.initial_mamba_size,
    ...
    def test_mamba_state_dtype_override(self):
        ...
        mamba_layers = [i for i in range(48) if i not in [3, 7, 11, 15, 19, 23, 27, 31, 35, 39, 43, 47]]
